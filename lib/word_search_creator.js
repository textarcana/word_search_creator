        ...
        for (row in matrix) {
            for (column in matrix[row]) {
                if (matrix[row][column] === target[character]) {
                    ...
                    if ((character < matrix[row].length - 1) && (column < matrix[row].length - 1) && 
                        (matrix[row][column + 1] === target[character + 1] || 
                         matrix[row][column + 1] === 0)) {
                        rightFit = true;
                        console.log('possible RIGHT-hand fit for %s: %s', target[character], matrix[row][parseInt(column) + 1])
                    }

                    if (leftFit === true && rightFit === true) {
                        matrix[row][column - 1] = target[character - 1];
                        matrix[row][column] = target[character];
                        matrix[row][parseInt(column) + 1] = target[parseInt(character) + 1];
                    }
